...
new bool:g_aActiveHoming[MAXPLAYERS+1] = false;
...
public void OnPluginStart()
{
	...
	cvar_defaulHomingState = CreateConVar("hbp_defaultHomingState", "1", "Default homing state for all players.", 0, true, 0.0, true, 1.0);
	cvar_defaultBounceState = CreateConVar("hbp_defaultBounceState", "1", "Default bouncy state for all players.", 0, true, 0.0, true, 1.0);
...
public OnEntityCreated(iProjectile, const char[] classname){
	if(StrEqual(classname, "tf_projectile_sentryrocket") || StrEqual(classname, "tf_projectile_energy_ring"))
	{
		RequestFrame(Frame_EntityCreated, EntIndexToEntRef(iProjectile));
	}
	return;
}

public void Frame_EntityCreated(int entref){
	if(!IsValidEntity(entref))
		return;
	int iLauncher = GetEntPropEnt(entref, Prop_Data, "m_hOwnerEntity");
	if(!HasEntProp(iLauncher, Prop_Send, "m_hBuilder"))
		return;
	...
	if(isFF2)
	{
		if(GetClientTeam(iLauncher)==FF2_GetBossTeam())
			return;
	}
	#endif
	int weapon = GetPlayerWeaponSlot(iLauncher, TFWeaponSlot_Secondary);
	if(!IsValidEntity(weapon))
		return;
	int wepindex = GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex");
	if(wepindex != 1086 && wepindex != 140)
		return;
	int iData[ARRAY_ENTITIE_SIZE];
	iData[entityRef] = entref;
	iData[target] = 0;
	iData[homing] = 0;
	iData[bouncy] = 0;
	iData[bounces] = 0;
	if(arrModifiedEntities.Length <= GetConVarInt(cvar_maxModifiedProjectiles))
	{
		...
		SDKHook(EntRefToEntIndex(entref), SDKHook_StartTouch, OnStartTouch);
	} else {
...
public OnGameFrame(){
	static bool:aPressed[MAXPLAYERS + 1] = { false, ... };
	...
	for(int i = arrModifiedEntities.Length -1; i >= 0; i--){
		if(arrModifiedEntities.GetArray(i, iData)){
			...
			else
			{
				...
				{
					new iOwner = GetEntPropEnt(iProjectile, Prop_Data, "m_hOwnerEntity");
					if(IsValidEntity(iOwner) && !IsValidClient(iOwner))
					{
						iOwner = GetEntPropEnt(iOwner, Prop_Send, "m_hBuilder");
					}
					if(IsValidClient(iOwner) && IsPlayerAlive(iOwner))
					{
						int iWeaponDefID = GetEntProp(GetEntPropEnt(iOwner, Prop_Send, "m_hActiveWeapon"), Prop_Send, "m_iItemDefinitionIndex");
						if(iData[homing] == 1 && (iWeaponDefID == 1086 || iWeaponDefID == 140))
						{
							if((GetClientButtons(iOwner) & IN_RELOAD))
							{
								if(!aPressed[iOwner])
								{
									g_aActiveHoming[iOwner] = !g_aActiveHoming[iOwner];
									PrintToChat(iOwner, "\x03Homing %sactivated!", ( g_aActiveHoming[iOwner] ? "" : "de" ));
								}
								aPressed[iOwner] = true;
							}
							else
								aPressed[iOwner] = false;
							if(g_aActiveHoming[iOwner])
							{
