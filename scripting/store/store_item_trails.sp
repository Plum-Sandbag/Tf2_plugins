...
stock int ShouldHideTrail(int client,int ent)
{
	if(GAME_TF2)
	{
		...
		{
			if(TF2_IsPlayerInCondition(g_iTrailOwners[ent], TFCond_Disguised) || TF2_IsPlayerInCondition(g_iTrailOwners[ent], TFCond_Cloaked)
			|| TF2_IsPlayerInCondition(g_iTrailOwners[ent], TFCond_Stealthed) || TF2_IsPlayerInCondition(g_iTrailOwners[ent], TFCond_CloakFlicker)
			|| TF2_IsPlayerInCondition(g_iTrailOwners[ent], TFCond_DeadRingered))
			{
				return 2;
			}
		}
