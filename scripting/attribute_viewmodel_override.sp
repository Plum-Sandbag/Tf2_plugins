...
void SDKHook_OnEntitySpawn(int entity)
{
	...
	{
		if(strlen(model) <= 3 && !FileExists(model, true))
			return;
		PrecacheModel(model);
...
void Event_OnObjectSapped(Event event, const char[] name, bool bDontBroadcast)
{
	...
	if(!TF2Attrib_HookValueString("", "set_weapon_model", sapper, model, sizeof(model)) 
			...
			&& !TF2CustAttr_GetString(sapper, "set weapon model", model, sizeof(model)) 
			&& !TF2CustAttr_GetString(sapper, "set weapon viewmodel", model, sizeof(model)))
		return;
...
void SDHook_OnWeaponEquipPost(int client, int weapon)
{
	...
	if(!TF2Attrib_HookValueString("", "set_viewmodel_animated_arms", weapon, model_path, sizeof(model_path)) || TF2CustAttr_GetString(weapon, "set viewmodel arms", model_path, sizeof(model_path)))
		return;
...
void OnDrawWeapon(int client, int weapon)
{
	...
	if(TF2Attrib_HookValueString("", "set_weapon_model", weapon, model, sizeof(model)) || TF2CustAttr_GetString(weapon, "set weapon model", model, sizeof(model)))
	{
		SetWeaponViewmodel(client, weapon, model);
		SetWeaponWorldmodel(client, weapon, model);
	}

	if(TF2Attrib_HookValueString("", "set_weapon_viewmodel", weapon, model, sizeof(model)) || TF2CustAttr_GetString(weapon, "set weapon viewmodel", model, sizeof(model)))
	{
		SetWeaponViewmodel(client, weapon, model);
	}
	
	if(TF2Attrib_HookValueString("", "set_weapon_worldmodel", weapon, model, sizeof(model)) || TF2CustAttr_GetString(weapon, "set weapon worldmodel", model, sizeof(model)))
	{
		SetWeaponWorldmodel(client, weapon, model);
	}

	if(TF2Attrib_HookValueString("", "set_viewmodel_bonemerged_arms", weapon, model, sizeof(model)) || TF2CustAttr_GetString(weapon, "set viewmodel bonemerged arms", model, sizeof(model)))
	{
...
stock void SetWeaponWorldmodel(int client, int weapon, char[] model_path = "", int model_id = 0)
{
	...
	if(strlen(model_path) <= 3 && !FileExists(model_path, true))
		return;
