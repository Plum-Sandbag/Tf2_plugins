...
bool canBossRTD;
float SniperDamage = 3.0;
float SniperMiniDamage = 3.0;
...
public void OnPluginStart()
{
	...
	cvarSniperDamage = CreateConVar("ff2_sniper_dmg", "3.0", "Sniper Rifle normal multiplier", _, true, 0.0);
	cvarSniperMiniDamage = CreateConVar("ff2_sniper_dmg_mini", "3.0", "Sniper Rifle mini-crit multiplier", _, true, 0.0);
...
public Action Command_MakeBoss(int client, int args)
{
	...
	for(int target; target<target_count; target++)
	{
		...
		if(IsBoss(target_list[target]))
		{
			...
			{
				BossSwitched[boss] = GetRandomUInt(0, 1) ? true : false;
			}
		...
		else
		{
			if(index >= 0)
			{
				...
				{
					BossSwitched[index] = GetRandomUInt(0, 1) ? true : false;
				}
			...
			else
			{
				while(boss2 <= MaxClients)
				{
					if(!IsValidClient(Boss[boss2]))
					{
						...
						{
							BossSwitched[boss] = GetRandomUInt(0, 1)==1;
						}
...
/*	char path[PLATFORM_MAX_PATH], filename[PLATFORM_MAX_PATH];
	...
	while(ReadDirEntry(directory, filename, sizeof(filename), filetype))
	{
		...
		{
//1			ServerCommand("sm plugins unload freaks/%s", filename);
//1			LogMessage("Stopped %s Plugin", filename);
		}
	}*/
}
...
//			LogMessage("Stopped %s Plugin", filename);
			RequestFrame(Subplugins_Load);
//			ServerCommand("sm plugins reload freaks/%s", filename);
		}
	}
}

public void Subplugins_Load() 
{
	char path[PLATFORM_MAX_PATH], filename[PLATFORM_MAX_PATH], fullfilename[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, path, PLATFORM_MAX_PATH, "plugins/freaks");
	FileType filetype;
	Handle directory = OpenDirectory(path);
	while(ReadDirEntry(directory, filename, sizeof(filename), filetype))
	{
		if(filetype==FileType_File && StrContains(filename, ".smx", false)!=-1)
		{
			Format(fullfilename, sizeof(fullfilename), "freaks/%s", filename);
			if(FindPluginByFile(fullfilename) == INVALID_HANDLE)
				ServerCommand("sm plugins load freaks/%s", filename);
		}
	}
}
		...
		else if(StrEqual(section, "mod_precache") || !StrContains(section, "sound_") || !StrContains(section, "catch_"))
		{
			for(int i=1; ; i++)
			{
				...
				if(StrEqual(section, "mod_precache"))
				{
					...
					{
//						PrintToServer("[mod_precache] Precaching %s.", file);
						PrecacheModel(file);
					}
					else
					{
...
public void OnRoundSetup(Event event, const char[] name, bool dontBroadcast)
{
	...
	currentBossTeam = GetRandomUInt(1, 2);
	switch(cvarForceBossTeam.IntValue)
	{
		case 1:
			blueBoss = view_as<bool>(GetRandomUInt(0, 1));
...
public void OnRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	for(int client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client))
		{
			...
			{
				FF2flags[client] &= ~FF2FLAG_CLASSTIMERDISABLED;
				playing++;
	...
	if(bosses==1 && kvDiffMods!=null && IsValidClient(Boss[0]) && IsPlayerAlive(Boss[0]))
	{
		...
		else if((IsFakeClient(Boss[0]) && !cvarDifficulty.BoolValue && !GetRandomUInt(0, 9)) || (GetRandomFloat(0.0, 100.0)<cvarDifficulty.FloatValue && ToggleDiff[Boss[0]]!=Setting_Off  && ToggleDiff[Boss[0]]!=Setting_Temp))
		{
			int count;
			KvRewind(kvDiffMods);
			...
			if(count)
			{
				KvRewind(kvDiffMods);
				KvGotoFirstSubKey(kvDiffMods);
				for(count=GetRandomUInt(0, count-1); count>0; count--)
				{
...
public void OnRoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		if(GetRandomUInt(0, 99) < cvarBossVsBoss.IntValue)
		{
...
public Action Timer_SetEnabled3(Handle timer, bool toggle)
{
	...
	if(Enabled3)
	{
		...
		for(int client=1; client<=MaxClients; client++)
		{
			...
			if(reds>blus || (reds==blus && GetRandomUInt(0, 1))) // More reds or their equal with 50/50 chance
			{
				team = BossTeam;
				reds--;
				blus++;
...
public Action Timer_PrepareBGM(Handle timer, any userid)
{
	...
	if(KvJumpToKey(BossKV[Special[0]], "sound_bgm"))
	{
		...
		for(int i; i<19; i++)
		{
			index = GetRandomUInt(1, topIndex-1);
...
void EquipBoss(int boss)
{
	...
	static char classname[64], wepModel[PLATFORM_MAX_PATH], arm[PLATFORM_MAX_PATH], wModel[PLATFORM_MAX_PATH];
	...
	for(int i=1; ; i++)
	{
		...
		if(KvJumpToKey(BossKV[Special[boss]], key))
		{
			...
			switch(strangerank)
			{
				case 0:
				{
					strangekills = GetRandomUInt(0, 9);
				}
				case 1:
				{
					strangekills = GetRandomUInt(10, 24);
				}
				case 2:
				{
					strangekills = GetRandomUInt(25, 44);
				}
				case 3:
				{
					strangekills = GetRandomUInt(45, 69);
				}
				case 4:
				{
					strangekills = GetRandomUInt(70, 99);
				}
				case 5:
				{
					strangekills = GetRandomUInt(100, 134);
				}
				case 6:
				{
					strangekills = GetRandomUInt(135, 174);
				}
				case 7:
				{
					strangekills = GetRandomUInt(175, 224);
				}
				case 8:
				{
					strangekills = GetRandomUInt(225, 274);
				}
				case 9:
				{
					strangekills = GetRandomUInt(275, 349);
				}
				case 10:
				{
					strangekills = GetRandomUInt(350, 499);
				}
				case 11:
				{
					if(index == 656)	// Holiday Punch is different
					{
						strangekills = GetRandomUInt(500, 748);
					}
					else
					{
						strangekills = GetRandomUInt(500, 749);
					}
				}
				case 12:
				{
					...
					{
						strangekills = GetRandomUInt(750, 998);
					}
				}
				case 13:
				{
					if(index == 656)
					{
						strangekills = GetRandomUInt(750, 999);
					}
					else
					{
						strangekills = 999;
				...
				case 14:
				{
					strangekills = GetRandomUInt(1000, 1499);
				}
				case 15:
				{
					strangekills = GetRandomUInt(1500, 2499);
				}
				case 16:
				{
					strangekills = GetRandomUInt(2500, 4999);
				}
				case 17:
				{
					strangekills = GetRandomUInt(5000, 7499);
				}
				case 18:
				{
					if(index == 656)
					{
						strangekills = GetRandomUInt(7500, 7922);
					}
					else
					{
						strangekills = GetRandomUInt(7500, 7615);
					}
				}
				case 19:
				{
					if(index == 656)
					{
						strangekills = GetRandomUInt(7923, 8499);
					}
					else
					{
						strangekills = GetRandomUInt(7616, 8499);
					}
				}
				case 20:
				{
					strangekills = GetRandomUInt(8500, 9999);
				}
				default:
				{
					strangekills = GetRandomUInt(0, 9999);
			...
			KvGetString(BossKV[Special[boss]], "wepmodel", wepModel, sizeof(wepModel));

			KvGetString(BossKV[Special[boss]], "arm", arm, sizeof(arm));

			weapon = FF2_SpawnWeapon(client, classname, index, weaponlevel, KvGetNum(BossKV[Special[boss]], "quality", QualityWep), attributes, true, wepModel, arm);
			...
			if(KvJumpToKey(BossKV[Special[boss]], key))
			{
				...
				switch(strangerank)
				{
					case 0:
					{
						...
						{
							strangekills = GetRandomUInt(0, 14);
						}
					}
					case 1:
					{
						if(index==133 || index==444 || index==655)	// Gunboats, Mantreads, or Spirit of Giving
						{
							strangekills = GetRandomUInt(1, 2);
						}
						else
						{
							strangekills = GetRandomUInt(15, 29);
						}
					}
					case 2:
					{
						...
						{
							strangekills = GetRandomUInt(3, 4);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(3, 6);
						}
						else
						{
							strangekills = GetRandomUInt(30, 49);
						}
					}
					case 3:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(5, 6);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(7, 11);
						}
						else
						{
							strangekills = GetRandomUInt(50, 74);
						}
					}
					case 4:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(7, 9);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(12, 19);
						}
						else
						{
							strangekills = GetRandomUInt(75, 99);
						}
					}
					case 5:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(10, 13);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(20, 27);
						}
						else
						{
							strangekills =  GetRandomUInt(100, 134);
					...
					case 6:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(14, 17);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(28, 36);
						}
						else
						{
							strangekills = GetRandomUInt(135, 174);
					...
					case 7:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(18, 22);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(37, 46);
						}
						else
						{
							strangekills = GetRandomUInt(175, 249);
						}
					}
					case 8:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(23, 27);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(47, 56);
						}
						else
						{
							strangekills = GetRandomUInt(250, 374);
						}
					}
					case 9:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(28, 34);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(57, 67);
						}
						else
						{
							strangekills = GetRandomUInt(375, 499);
						}
					}
					case 10:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(35, 49);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(68, 78);
						}
						else
						{
							strangekills = GetRandomUInt(500, 724);
						}
					}
					case 11:
					{
						...
						{
							strangekills = GetRandomUInt(79, 90);
						}
						else
						{
							strangekills = GetRandomUInt(725, 999);
						}
					}
					case 12:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(75, 98);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(91, 103);
						}
						else
						{
							strangekills = GetRandomUInt(1000, 1499);
					...
					case 13:
					{
						...
						{
							strangekills = GetRandomUInt(104, 119);
						}
						else
						{
							strangekills = GetRandomUInt(1500, 1999);
						}
					}
					case 14:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(100, 149);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(120, 137);
						}
						else
						{
							strangekills = GetRandomUInt(2000, 2749);
						}
					}
					case 15:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(150, 249);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(138, 157);
						}
						else
						{
							strangekills = GetRandomUInt(2750, 3999);
						}
					}
					case 16:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(250, 499);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(158, 178);
						}
						else
						{
							strangekills = GetRandomUInt(4000, 5499);
						}
					}
					case 17:
					{
						...
						{
							strangekills = GetRandomUInt(179, 209);
						}
						else
						{
							strangekills = GetRandomUInt(5500, 7499);
						}
					}
					case 18:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(750, 783);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(210, 249);
						}
						else
						{
							strangekills = GetRandomUInt(7500, 9999);
						}
					}
					case 19:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(784, 849);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(250, 299);
						}
						else
						{
							strangekills = GetRandomUInt(10000, 14999);
						}
					}
					case 20:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(850, 999);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(300, 399);
						}
						else
						{
							strangekills = GetRandomUInt(15000, 19999);
						}
					}
					default:
					{
						if(index==133 || index==444)	// Gunboats or Mantreads
						{
							strangekills = GetRandomUInt(0, 999);
						}
						else if(index==655)	// Spirit of Giving
						{
							strangekills = GetRandomUInt(0, 399);
						}
						else
						{
							strangekills = GetRandomUInt(0, 19999);
						}
				...
				if(StrEqual(classname, "tf_wearable_demoshield"))
				{
					...
					if (IsValidEntity(wearable))
					{
						...
						SetEntData(wearable, FindSendPropInfo("CTFWearable", "m_iEntityQuality"), 5);
					...
					char atts[32][32];
					int attributeCount = ExplodeString(attributes, ";", atts, sizeof(atts), sizeof(atts[]));
					if(attributes[0] != '\0')
					{
						...
						{
							int attribIndex = StringToInt(atts[loop]);
							float temp = StringToFloat(atts[loop + 1]);
...
//					KvGetString(BossKV[Special[boss]], "wepmodel", wepModel, sizeof(wepModel));
//					if(wepModel[0])
//						TF2Attrib_SetFromStringValue(weapon, "set weapon model", wepModel);
//						
//					KvGetString(BossKV[Special[boss]], "arm", arm, sizeof(arm));
//					if(arm[0])
//						TF2Attrib_SetFromStringValue(weapon, "set viewmodel arms", arm);
...
stock bool ConfigureWorldModelOverride(int entity, const char[] model, bool wearable=false)
{
	if(!FileExists(model, true))
		return false;
//	PrintToServer("[WorldModel] Precaching %s.", model);
...
stock int GetRandBlockCell(ArrayList hArray, int &iSaveIndex, int iBlock=0, bool bAsChar=false, int iDefault=0)
{
	...
	if(iSize > 0)
	{
		iSaveIndex = GetRandomUInt(0, iSize - 1);
...
public Action OnObjectDestroyed(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(GetRandomUInt(0, 2) || !IsBoss(attacker))
		return Plugin_Continue;
...
//	static char pluginName[PLATFORM_MAX_PATH];
	static char pluginName[80], filepath[PLATFORM_MAX_PATH], fullpluginName[PLATFORM_MAX_PATH];
...
//	ReplaceString(pluginName, sizeof(pluginName), "freaks/", " ", false);
	...
	Format(fullpluginName, sizeof(fullpluginName), "freaks/%s.smx", pluginName);
	if(FindPluginByFile(fullpluginName) == INVALID_HANDLE)
	...
	for(client=1; client<=MaxClients; client++)
	{
		...
		if((alive || IsClientObserver(client)) && !HudSettings[client][0] && !(FF2flags[client] & FF2FLAG_HUDDISABLED) && !(buttons & IN_SCORE))
		{
			...
			if(alive && LookHud!=0)
			{
				...
				if(observer)
					GetClientName(observer, playername, sizeof(playername));
			}
			...
			if(IsValidClient(observer))
			{
				if(StatHud>-1 && (CheckCommandAccess(client, "ff2_stats_bosses", ADMFLAG_BAN, true) || StatHud>1))
				{
					if(IsBoss(observer))
					{
						ShowSyncHudText(client, statHUD, "%s%t", top, "Player Stats Boss", playername, BossWins[observer], BossLosses[observer], BossKills[observer], BossDeaths[observer]);
					}
					else if((Healing[observer]>0 && HealHud==1) || HealHud>1)
					{
						ShowSyncHudText(client, statHUD, "%s%t", top, "Player Stats Healing", playername, Damage[observer], Healing[observer], PlayerKills[observer], PlayerMVPs[observer]);
					}
					else
					{
						ShowSyncHudText(client, statHUD, "%s%t", top, "Player Stats", playername, Damage[observer], PlayerKills[observer], PlayerMVPs[observer]);
					}
				}
				else if(!IsBoss(observer))
				{
					...
					{
						ShowSyncHudText(client, statHUD, "%s%t", top, "Spectator Damage Dealt", playername, Damage[observer], "Healing", Healing[observer]);
					}
					else
					{
						ShowSyncHudText(client, statHUD, "%s%t", top, "Spectator Damage Dealt", playername, Damage[observer]);
					}
				}
				else
				{
...
//	PrintToServer("[strPlayerModel] Precaching %s.", strPlayerModel);
	int iPlayerModel = PrecacheModel(strPlayerModel, true);
//	PrintToServer("[strEnemyModel] Precaching %s.", strEnemyModel);
...
public void TF2_OnConditionRemoved(int client, TFCond condition)
{
	...
	if(TF2_GetPlayerClass(client)==TFClass_Spy && !IsBoss(client) && (condition==TFCond_Cloaked || condition==TFCond_Stealthed))
	{
		if(GetClientCloakIndex(client) != 59)
			SetEntPropFloat(client, Prop_Send, "m_flCloakMeter", 0.0); //remove cloak meter while uncloak.
	}
}
...
public Action Timer_CheckAlivePlayers(Handle timer)
{
	...
	if(MercAlivePlayers==1 && BossAlivePlayers && Boss[0] && playingmerc>1 && !DrawGameTimer && LastMan && !Enabled3)
	{
		...
		for(int i=1; i<=MaxClients; i++)
		{
			...
			{
				TF2Attrib_SetByDefIndex(i,16,90);
			}
		}
	}
	...
	if(PointType!=1 && AliveToEnable>0 && !executed)
	{
		...
		if(alivePlayers < playing)
		{
			...
			{
				char sound[64];
				FormatEx(sound, sizeof(sound), "Announcer.AM_CapIncite0%i", GetRandomUInt(1, 4));
			...
			case 3:
			{
				if(GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee)!=weapon && shieldHP[client]>0 && damage<preHealth)
				{
					...
					if(shieldHP[client] > 0)
					{
						...
						FormatEx(ric, sizeof(ric), "weapons/fx/rics/ric%i.wav", GetRandomUInt(1,5));
			...
			switch(TF2_GetPlayerClass(client))
 			{
 				case TFClass_Spy:
 				{
					...
					if(damage >= 75.0)  //Temporary stopgap for small amounts of damage still doing 62 health
 					{
			...
			if(attacker <= MaxClients)
			{
				...
				if(!StrContains(classname, "tf_weapon_sniperrifle"))
				{
					if(CheckRoundState() != 2)
					{
						...
						else if(index!=230 && index!=402 && index!=526 && index!=30665 && index!=1098)  //Sydney Sleeper, Bazaar Bargain, Machina, Shooting Star
						{
				...
				switch(index)
				{
					case 21, 40, 208, 215, 659, 798, 807, 887, 896, 905, 914, 963, 972, 1146, 30474, 1178://喷火加血
					{
						...
						if (health < max+85)
						{
							if (newhealth > max+85) newhealth = max+85;
							SetEntProp(attacker, Prop_Data, "m_iHealth", newhealth);
							SetEntProp(attacker, Prop_Send, "m_iHealth", newhealth);
						}
					}
					case 15, 298, 41, 312, 424, 654, 793, 802, 882, 891, 900, 909, 958, 967, 811://机枪加血
					{
						int health = GetClientHealth(attacker);
						int max = GetEntProp(attacker, Prop_Data, "m_iMaxHealth");
						int newhealth = health+5;
						if(health < max+150)
						{
							if (newhealth > max+150) newhealth = max+150;
							SetEntProp(attacker, Prop_Data, "m_iHealth", newhealth);
							SetEntProp(attacker, Prop_Send, "m_iHealth", newhealth);
						}
...
//								damage = ((Pow(float(BossHealthMax[boss]), 0.74074)+512.0-(Marketed[client]/128.0*float(BossHealthMax[boss])))/3)*bosses*cvarMarket.FloatValue;
				...
				if(bIsBackstab)
				{
					...
					{
						damage = Pow((BossHealthMax[boss]*bosses*(LastBossIndex()+1)*BossLivesMax[boss]*(0.11-Stabbed[boss]/90)+(1500/float(playing)) + 700.0)/3,0.94);
					}
					else
					{
					...
					switch(index)
					{
						...
						case 461:	//Big Earner
						{
							...
							TF2_AddCondition(attacker, TFCond_SpeedBuffAlly, 6.5);  //Speed boost
						}
					}
...
public Action OnStomp(int attacker, int victim, float &damageMultiplier, float &damageBonus, float &JumpPower)
{
	...
	if(IsBoss(victim))
	{
		...
		int lives = BossLives[GetBossIndex(victim)] - 1;
		if(lives >= 0)
		{
			damageBonus += (lives * BossHealthMax[GetBossIndex(victim)] * damageMultiplier);
//			int health = GetClientHealth(victim);
//			float damage = health * damageMultiplier + damageBonus;
//			PrintToChatAll("lives:%d, maxhealth:%d, damageMultiplier:%f, damage:%f, health:%d", lives, BossHealthMax[GetBossIndex(victim)], damageMultiplier, damage, health);
		}
		return Plugin_Changed;
	}
...
stock int SpawnSmallHealthPackAt(int client, int team=0, int attacker)
{
	...
	if(IsValidEntity(healthpack))
	{
		...
		float velocity[3];//={float(GetRandomUInt(-10, 10)), float(GetRandomUInt(-10, 10)), 50.0};  //Q_Q
		velocity[0] = float(GetRandomUInt(-10, 10));
		velocity[1] = float(GetRandomUInt(-10, 10));
...
stock void RandomlyDisguise(int client)	//Original code was mecha's, but the original code is broken and this uses a better method now.
{
	if(IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		{
			disguiseTarget = disguiseArray.Get(GetRandomUInt(0, disguiseArray.Length-1));
		...
		{
			TF2_DisguisePlayer(client, view_as<TFTeam>(team), GetRandomUInt(0, 1) ? TFClass_Medic : TFClass_Scout, disguiseTarget);
		}
		else
		{
			TF2_AddCondition(client, TFCond_Disguised, -1.0);
			SetEntProp(client, Prop_Send, "m_nDisguiseTeam", team);
			SetEntProp(client, Prop_Send, "m_nDisguiseClass", GetRandomUInt(0, 1) ? view_as<int>(TFClass_Medic) : view_as<int>(TFClass_Scout));
...
public Action TF2_CalcIsAttackCritical(int client, int weapon, char[] weaponname, bool &result)
{
	...
	if(!IsValidClient(client))
		return Plugin_Continue;
	if(TF2_GetPlayerClass(client)==TFClass_Heavy || TF2_GetPlayerClass(client)==TFClass_Medic || TF2_GetPlayerClass(client)==TFClass_Sniper || TF2_GetPlayerClass(client)==TFClass_Engineer)
	{
		int melee=GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
		if(melee==weapon)
		{
			int wepindex=GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex");
			if(wepindex!=239 && wepindex!=1084 && wepindex!=1100)
...
stock int GetRandomValidClient(bool[] omit)
{
	...
	for(int client=1; client<=MaxClients; client++)
	{
		if(IsValidClient(client) && !omit[client] && (QueuePoints[client]>=QueuePoints[companion] || (cvarDuoRandom.BoolValue && !GetRandomUInt(0, RoundToCeil(MaxClients/5.0)))))
		{
...
stock bool RandomSound(const char[] sound, char[] file, int length, int boss=0)
{
	...
	static char temp[6];
	int choosen = GetRandomUInt(1, sounds);
...
stock bool RandomSoundAbility(const char[] sound, char[] file, int length, int boss=0, int slot=0)
{
	...
	IntToString(match[GetRandomUInt(0, matches-1)], key, 4);
	KvGetString(BossKV[Special[boss]], key, file, length);  //Populate file
	return true;
}
	...
	if(boss == companion)
	{
		...
		for(int tries; tries<100; tries++)
		{
			if(ChancesString[0])
			{
				...
				int i = GetRandomUInt(0, chances[characterIndex-1]);
			...
			{
				Special[boss] = GetRandomUInt(0, Specials-1);
			}
...
public Action Timer_DisplayCharsetVote(Handle timer)
{
	...
	if(shuffle)
	{
		...
		{
			choosen = GetRandomUInt(0, packs);	// Get a random pack
	...
	{
		IntToString(validCharsets[GetRandomUInt(0, charsets-1)], index, sizeof(index));
...
stock void RemoveShield(int client, int attacker)
{
	...
	{
		shield[client] = 0;
		CreateTimer(0.3, Timer_StopCharging, GetClientUserId(client), TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
		CreateTimer(60.0, Timer_Regshield, client, TIMER_FLAG_NO_MAPCHANGE);
//		TF2_RemoveWearable(client, shield[client]);
		EmitSoundToClient(client, "player/spy_shield_break.wav", _, _, _, _, 0.7, _, _, _, _, false);
		EmitSoundToClient(attacker, "player/spy_shield_break.wav", _, _, _, _, 0.7, _, _, _, _, false);
...
public Action Timer_StopCharging(Handle timer, int userid)
{
	int client = GetClientOfUserId(userid);
	if(!IsValidClient(client))
		return Plugin_Stop;
...
public Action Timer_Regshield(Handle timer, int client)
{
	int entity = -1;
	while((entity=FindEntityByClassname2(entity, "tf_wearable_demoshield")) != -1)  //Demoshields
	{
		if(GetEntPropEnt(entity, Prop_Send, "m_hOwnerEntity")==client && !GetEntProp(entity, Prop_Send, "m_bDisguiseWearable"))
		{
			FF2_SetClientShield(client, entity, 100.0);
		}
	}
}
...
stock int FF2_SpawnWeapon(int client, char[] name, int index, int level, int qual, const char[] att, bool visible=true, const char[] wepModel="", const char[] arm="")
{
	...
	if(strlen(wepModel) > 3 && FileExists(wepModel, true))
	{
//		PrintToServer("[wepModel] Precaching %s.", wepModel);
		PrecacheModel(wepModel, true);
		TF2Attrib_SetFromStringValue(entity, "set weapon model", wepModel);
	}
	if(strlen(arm) > 3 && FileExists(arm, true))
	{
//		PrintToServer("[arm] Precaching %s.", arm);
		if(PrecacheModel(arm, true))
			TF2Attrib_SetFromStringValue(entity, "set viewmodel arms", arm);
	}
...
stock int GetRandomUInt(int min, int max)
{
