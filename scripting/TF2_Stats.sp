...
new Handle:loose_cannon_reflect = INVALID_HANDLE;
new Handle:medigun_shield = INVALID_HANDLE;
new Handle:rocketlauncher = INVALID_HANDLE;
new Handle:shooting_star = INVALID_HANDLE;
new Handle:the_capper = INVALID_HANDLE;
new Handle:prinny_machete = INVALID_HANDLE;
new Handle:batsaber = INVALID_HANDLE;
...
createdbplayer()
{
	...
	len += Format(query[len], sizeof(query)-len, "`loose_cannon_reflect` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`rocketlauncher` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`shooting_star` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`the_capper` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`prinny_machete` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`batsaber` int(11) NOT NULL DEFAULT '0',");
...
createdbplayersqllite()
{
	...
	len += Format(query[len], sizeof(query)-len, ", `loose_cannon_reflect` INTEGER DEFAULT 0");
	len += Format(query[len], sizeof(query)-len, ", `rocketlauncher` INTEGER DEFAULT 0");
	len += Format(query[len], sizeof(query)-len, ", `shooting_star` INTEGER DEFAULT 0");
	len += Format(query[len], sizeof(query)-len, ", `the_capper` INTEGER DEFAULT 0");
	len += Format(query[len], sizeof(query)-len, ", `prinny_machete` INTEGER DEFAULT 0");
	len += Format(query[len], sizeof(query)-len, ", `batsaber` INTEGER DEFAULT 0");
...
public CreateCvars()
{
	...
	loose_cannon_reflect = CreateConVar("rank_loose_cannon_reflect_points","2","Points:Pyro - The loose cannon Reflect");
	medigun_shield = CreateConVar("rank_medigun_shield_points","2","Points:Medic - The Medigun Shield");
	rocketlauncher = CreateConVar("rank_rocketlauncher_points","2","Points:Soldier - The Rocket Launcher");
	shooting_star = CreateConVar("rank_shooting_star_points","2","Points:Sniper - Shooting Star");
	the_capper = CreateConVar("rank_the_capper_points","2","Points:Generic - C.A.P.P.E.R");
	prinny_machete = CreateConVar("rank_prinny_machete_points","2","Points:Generic - Prinny Machete");
	batsaber = CreateConVar("rank_batsaber_points","2","Points:Scout - Batsaber");
			...
			if (attacker != 0)
			{
				if (attacker != victim)
				{
					...
					*/
					...
					else if (strcmp(weapon[0], "loose_cannon_reflect", false) == 0)
					{
						pointvalue = GetConVarInt(loose_cannon_reflect)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, loose_cannon_reflect = loose_cannon_reflect + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
					else if (strcmp(weapon[0], "rocketlauncher", false) == 0)
					{
						pointvalue = GetConVarInt(rocketlauncher)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, rocketlauncher = rocketlauncher + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
					else if (strcmp(weapon[0], "shooting_star", false) == 0)
					{
						pointvalue = GetConVarInt(shooting_star)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, shooting_star = shooting_star + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
					else if (strcmp(weapon[0], "the_capper", false) == 0)
					{
						pointvalue = GetConVarInt(the_capper)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, the_capper = the_capper + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
					else if (strcmp(weapon[0], "prinny_machete", false) == 0)
					{
						pointvalue = GetConVarInt(prinny_machete)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, prinny_machete = prinny_machete + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
					else if (strcmp(weapon[0], "batsaber", false) == 0)
					{
						pointvalue = GetConVarInt(batsaber)
						Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, batsaber = batsaber + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
						ExeQuery = true;
					}
		...
		else
		{
			...
			if (tmpdbversion <= 44)
			{
				if (!sqllite)
				{
					...
					new len = 0;
					decl String:query[1000];
					len += Format(query[len], sizeof(query)-len, "ALTER TABLE `Player` ADD `loose_cannon_reflect` int(11) NOT NULL DEFAULT '0',");
					len += Format(query[len], sizeof(query)-len, "ADD `rocketlauncher` int(11) NOT NULL DEFAULT '0',");
					len += Format(query[len], sizeof(query)-len, "ADD `shooting_star` int(11) NOT NULL DEFAULT '0',");
					len += Format(query[len], sizeof(query)-len, "ADD `the_capper` int(11) NOT NULL DEFAULT '0',");
					len += Format(query[len], sizeof(query)-len, "ADD `prinny_machete` int(11) NOT NULL DEFAULT '0';");
					SQL_TQuery(db, SQLErrorCheckCallback, query);

					Format(buffer, sizeof(buffer), "UPDATE data SET dataint = 45 where `name` = 'dbversion'");
					SQL_TQuery(db,SQLErrorCheckCallback, buffer);
				}
				else
				{
					new String:buffer[600];
					Format(buffer, sizeof(buffer), "ALTER table Player add loose_cannon_reflect INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
					Format(buffer, sizeof(buffer), "ALTER table Player add rocketlauncher INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
					Format(buffer, sizeof(buffer), "ALTER table Player add shooting_star INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
					Format(buffer, sizeof(buffer), "ALTER table Player add the_capper INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
					Format(buffer, sizeof(buffer), "ALTER table Player add prinny_machete INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
			...
			if (tmpdbversion <= 45)
			{
				if (!sqllite)
				{
					...
					new len = 0;
					decl String:query[1000];
					len += Format(query[len], sizeof(query)-len, "ALTER TABLE `Player` ADD `batsaber` int(11) NOT NULL DEFAULT '0';");
					SQL_TQuery(db, SQLErrorCheckCallback, query);

					Format(buffer, sizeof(buffer), "UPDATE data SET dataint = 46 where `name` = 'dbversion'");
					SQL_TQuery(db,SQLErrorCheckCallback, buffer);
				}
				else
				{
					new String:buffer[600];
					Format(buffer, sizeof(buffer), "ALTER table Player add batsaber INTEGER DEFAULT 0 ;")
					SQL_TQuery(db, SQLErrorCheckCallback, buffer);
