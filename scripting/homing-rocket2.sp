...
public OnGameFrame()
{
	...
	for( new iClient = 1; iClient <= MaxClients; iClient++ )
	{
		if( IsClientInGame(iClient) && IsPlayerAlive(iClient) && (g_nAdminFlags!=0 && (GetUserFlagBits(iClient) & g_nAdminFlags)!=0 || g_nAdminFlags==0) )
		{
			...
			b5000 = (StrContains(sWeaponName, "tf_weapon_particle_cannon", false) != -1);
			bLaserPointer = (StrContains(sWeaponName, "tf_weapon_laser_pointer", false) != -1);
			if( b5000 && (g_nHomingMode==1 || g_nHomingMode==3) || bRocketLauncher && (g_nHomingMode==1 || g_nHomingMode==3) || bLaserPointer && (g_nHomingMode==2 || g_nHomingMode==3) )
			{
				// toggle homing
				if( g_nForceHoming == 1 && b5000 || g_nForceHoming == 1 && bRocketLauncher || g_nSForceHoming == 0 && bLaserPointer )
					if( b5000 && (GetClientButtons(iClient) & IN_ATTACK2) || bRocketLauncher && (GetClientButtons(iClient) & IN_ATTACK2) || bLaserPointer && (GetClientButtons(iClient) & IN_RELOAD) )
					{
						if(!bPressed[iClient])
						{
				...
				if( g_bShowAim && b5000 && ( g_bActiveHoming[iClient] && g_nForceHoming==1 || g_nForceHoming==2 ) || g_bShowAim && bRocketLauncher && ( g_bActiveHoming[iClient] && g_nForceHoming==1 || g_nForceHoming==2 ) || g_bSShowAim && bLaserPointer && ( g_bActiveHoming[iClient] && g_nSForceHoming==0 || g_nSForceHoming==1 ) )
				{
...
public OnEntityCreated(entity, const String:sClassName[])
{
	if( g_bPluginOn )
		...
		else if(StrEqual(sClassName,"tf_projectile_energy_ball") && (g_nHomingMode==1 || g_nHomingMode==3))
			SDKHook(entity, SDKHook_Spawn, Hook_OnEnergySpawn);
...
//////////////////////////////////////////////////
public Hook_OnEnergySpawn(entity)
	if( g_bPluginOn )
		if( g_iRocketOwners[entity] <= 0 )
		{
			new iOwner = GetEntPropEnt(entity, Prop_Data, "m_hOwnerEntity");
	...
	if( g_bPluginOn )
	{
		if( IsValidEntity(entity) )
		{
			if( g_iRocketOwners[entity] > 0 )
			{
				...
				if ( StrContains(sWeaponName, "tf_weapon_particle_cannon", false) == -1 && StrContains(sWeaponName, "tf_weapon_rocketlauncher", false) == -1 && StrContains(sWeaponName, "tf_weapon_laser_pointer", false) == -1 )
					return Plugin_Continue;
				
				if( !g_bAllowAllLaunchers && (StrContains(sWeaponName, "tf_weapon_particle_cannon", false) != -1 || StrContains(sWeaponName, "tf_weapon_rocketlauncher", false) != -1 ) )
				{
...
public Action:Command_ToggleHoming(iClient, iArgs)
{
	if( iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient) )
	{
		...
		new String:sWeaponName[32], bool:bLaserPointer, bool:bRocketLauncher, bool:b5000;
		GetClientWeapon(iClient, sWeaponName, sizeof(sWeaponName));
		bRocketLauncher = (StrContains(sWeaponName, "tf_weapon_rocketlauncher", false) != -1);
		b5000 = (StrContains(sWeaponName, "tf_weapon_particle_cannon", false) != -1);
		bLaserPointer = (StrContains(sWeaponName, "tf_weapon_laser_pointer", false) != -1);
		if( b5000 && (g_nHomingMode==1 || g_nHomingMode==3) && g_nForceHoming == 1 || bRocketLauncher && (g_nHomingMode==1 || g_nHomingMode==3) && g_nForceHoming == 1 || bLaserPointer && (g_nHomingMode==2 || g_nHomingMode==3) && g_nSForceHoming == 1 )
