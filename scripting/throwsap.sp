...
new Float:SapperRegenerateTime[MAXPLAYERS + 1];

new Handle:g_cvEnabled = INVALID_HANDLE;
new Handle:g_cvSapRadius = INVALID_HANDLE;
new Handle:g_cvSapStun = INVALID_HANDLE;
new Handle:g_cvSapCooldown = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_cvEnabled = CreateConVar("throwsap_enabled", "1", "Enable/Disable throwsap plugin.", _, true, 0.0, true, 1.0);
	g_cvSapRadius = CreateConVar("throwsap_sapradius", "300.0", "Radius of effect.");
	g_cvSapStun = CreateConVar("throwsap_saprstun", "0.2", "Sapper stun duration.");
	g_cvSapCooldown = CreateConVar("throwsap_sapcooldown", "50.0", "Sapper cooldown.");
...
public OnClientPutInServer(client)
{
	...
	SapperRegenerateTime[client] = 0.0;
//	tChargeTimer[client] = CreateTimer(0.5, Timer_ChargeMe, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
...
//	ClearTimer(tChargeTimer[client]);
	SDKUnhook(client, SDKHook_WeaponCanSwitchTo, OnClientWeaponCanSwitchTo);
...
public void OnPlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		tChargeTimer[client] = CreateTimer(0.5, Timer_ChargeMe, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
public Action:Timer_ChargeMe(Handle:timer, any:userid)
{
	int client = GetClientOfUserId(userid);
	if(timer != tChargeTimer[client])
		return Plugin_Stop;
	if (!bEnabled || !IsValidClient(client) || !IsPlayerAlive(client))
	{
		tChargeTimer[client] = INVALID_HANDLE;
		return Plugin_Stop;
	}

	if (TF2_GetPlayerClass(client) == TFClass_Spy && IsValidEntity(GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary)) && !ThrownSapperExists(client)) //If they are a spy, and have a sapper still
	{
/*        if (g_CalCharge[client] > 100)
		...
		if(GetClientTeam(client) == TEAM_RED)
			SetHudTextParams(0.85, 0.12, 0.6, 255, 0, 0, 255);
		if(GetClientTeam(client) == TEAM_BLU)
			SetHudTextParams(0.85, 0.12, 0.6, 0, 0, 255, 255);

		new Float:SapperCharge;

		if(SapperRegenerateTime[client])
		{
			if(SapperRegenerateTime[client] < GetEngineTime())
				SapperCharge = 100.0;
			else
				SapperCharge = 100.0 - (SapperRegenerateTime[client] - GetEngineTime()) * 2.0;
		}
		else
			SapperCharge = 100.0;
		...
		{
//            ShowSyncHudText(client, hHudCharge, "Sapper: %d%", g_CalCharge[client]);
			ShowSyncHudText(client, hHudCharge, "Sapper: %0.1f%", SapperCharge);
		}
	}
}
...
//                CreateTimer(50.0, Sapper_Resupply, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
				SapperRegenerateTime[client] = GetEngineTime() + GetConVarFloat(g_cvSapCooldown);
...
/*public Action:Sapper_Resupply(Handle:hTimer, any:userid)
{
	new client = GetClientOfUserId(userid);
	if (IsValidClient(client) && IsPlayerAlive(client))
	{
...
stock ThrowSapper(client, index)
{
	...
	if (IsValidEntity(sapper))
	{
		...
		tTimerLoop[client] = CreateTimer(0.25, LoopSapping, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
public Action:LoopSapping(Handle:timer, any:userid)
{
	...
	if (IsValidClient(client) && ThrownSapperExists(client) && IsPlayerAlive(client))
	{
		...
		new String:Name[32];
		Name = ((GetClientTeam(client) == _:TFTeam_Red) ? "electrocuted_blue" : "electrocuted_red");
		new Float:stunduration = GetConVarFloat(g_cvSapStun);
		...
		for (new clientIdx = 1; clientIdx <= MaxClients; clientIdx++)
		{
			...
			if(clientIdx == client)
				continue;
			if(GetClientTeam(clientIdx) == GetClientTeam(client))
				continue;

			GetEntPropVector(clientIdx, Prop_Data, "m_vecAbsOrigin", vPlayerPos);
			if (GetVectorDistance(vPlayerPos, vSapperPos) <= radius)
			{
				new Float:flCloak = GetEntPropFloat(clientIdx, Prop_Send, "m_flCloakMeter");

				flCloak -= 3.0;
				if (flCloak < 0.0) flCloak = 0.0;

				SetEntPropFloat(clientIdx, Prop_Send, "m_flCloakMeter", flCloak);
				
				new Float:fPunch[3];
				fPunch[0] = GetRandomFloat(-20.0, 20.0);
				fPunch[1] = GetRandomFloat(-20.0, 20.0);
				fPunch[2] = GetRandomFloat(-20.0, 20.0);
				SetEntPropVector(clientIdx, Prop_Send, "m_vecPunchAngle", fPunch);
				
				SDKHooks_TakeDamage(clientIdx, client, client, 10.0, DMG_SHOCK | DMG_PREVENT_PHYSICS_FORCE);
				
//				new Float:stunduration = GetConVarFloat(g_cvSapStun);
				
				TF2_StunPlayer(clientIdx, stunduration, 0.0, TF_STUNFLAGS_SMALLBONK|TF_STUNFLAG_NOSOUNDOREFFECT, client);
				...
				TE_WriteNum("entindex", clientIdx);
				TE_WriteNum("m_iAttachType", 1); // follow point or just follow
				TE_SendToAll(-1.0);
			}
		}
