        ...
        if (ref != 0 && IsClientInGame(i))
        {
            ...
            if (ent > 0)
            {
                ...
                else
                {
                    ...
                    if (buttons & IN_JUMP)
                    {
                        new flags = GetEntityFlags(ent);
                        if (flags & FL_ONGROUND)
                            vel[2] += (clientJumpSpeed[i] > 0.0) ? clientJumpSpeed[i] : defaultJumpSpeed;
                    }

                    TeleportEntity(ent, NULL_VECTOR, angles, vel);

                    /*
                    new Float:objectpos[3];
                    GetEntPropVector(ent, Prop_Send, "m_vecOrigin", objectpos);
    ...
    else
    {
        ...
        if (objectid <= 0 && ((permissions & REMOTE_CAN_BUILD) != 0))
        {
                 ...
                 CountObjects(client,TF2_ObjectClassNames[type], TF2_ObjectModes[type]) <= 0))
            {
                ...
                ClientCommand(client, "build %d %d", ent, mode);
            }
            else
            {
...
public Action:Activate(Handle:timer,any:ref)
{
    ...
    if (ent > 0 && IsValidEdict(ent) && IsValidEntity(ent))
    {
        ...
        if (TF2_GetObjectType(ent) != TFObject_Teleporter &&
            GetEntProp(ent, Prop_Send, "m_CollisionGroup") != 0)
        {
...
public Action:RemoteOff(client, args)
{
    ...
    if (objectRef != 0)
    {
        new ent = EntRefToEntIndex(objectRef);
        if (ent > 0 && IsValidEdict(ent) && IsValidEntity(ent))
        {
            new Float:angles[3];
                ...
                IsPlayerAlive(client))
            {
                ...
                if (distance < size[0] * -1.1 || distance > size[0] * 1.1)
                {
                    ...
                    //CreateTimer(2.0, Activate, EntIndexToEntRef(ent), TIMER_FLAG_NO_MAPCHANGE);
                }
            }
        }
...
public Action:RemoteGod(client, args)
{
    ...
    else
    {
        ...
        if (ent > 0 && IsValidEdict(ent) && IsValidEntity(ent))
        {
            if (GetEntProp(ent, Prop_Send, "m_takedamage", 1)) // mortal
            {
                SetEntProp(ent, Prop_Send, "m_takedamage", 0, 1);
                PrintToChat(client,"\x01\x04Building god mode on");
            }
            else // godmode
            {
                SetEntProp(ent, Prop_Send, "m_takedamage", 1, 1);
...
public Action:Command_Build(client, args)
{
           ...
           (GetUserFlagBits(client) & (ADMFLAG_GENERIC|ADMFLAG_ROOT)) != 0))))
    {
        ...
        new TFExtObjectType:ent = TFExtObjectType:StringToInt(sObject);
        new mode = StringToInt(sMode);

        new TFTeam:team = TFTeam:GetClientTeam(client);
        if (ent < TFExtObject_Dispenser || ent > TFExtObject_TeleporterExit || team < TFTeam_Red)
            return Plugin_Continue;

        new iCount = 0;
        if (!CheckBuild(client, ent, mode, iCount))
            return Plugin_Handled;
