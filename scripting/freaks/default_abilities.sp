	...
	for(new target=1; target<=MaxClients; target++)
	{
		if(IsClientInGame(target) && IsPlayerAlive(target) && GetClientTeam(target)!=BossTeam)
		{
			...
			if(!TF2_IsPlayerInCondition(target, TFCond_Ubercharged) && (GetVectorDistance(bossPosition, targetPosition)<=distance))
			{
				...
				CreateTimer(duration, RemoveEntity1, EntIndexToEntRef(AttachParticle(target, "yikes_fx", 75.0)), TIMER_FLAG_NO_MAPCHANGE);
			}
		}
	}
}
...
Rage_StunSentry(const String:ability_name[], boss)
{
	...
	while((sentry=FindEntityByClassname(sentry, "obj_sentrygun"))!=-1)
	{
		...
		if(GetVectorDistance(bossPosition, sentryPosition)<=distance)
		{
			...
			CreateTimer(duration, RemoveEntity1, EntIndexToEntRef(AttachParticle(sentry, "yikes_fx", 75.0)), TIMER_FLAG_NO_MAPCHANGE);
...
Charge_Teleport(const String:ability_name[], boss, slot, status)
{
	...
	switch(status)
	{
		...
		case 3:
		{
			...
			{
				CreateTimer(3.0, RemoveEntity1, EntIndexToEntRef(AttachParticle(client, particle)), TIMER_FLAG_NO_MAPCHANGE);
				CreateTimer(3.0, RemoveEntity1, EntIndexToEntRef(AttachParticle(client, particle, _, false)), TIMER_FLAG_NO_MAPCHANGE);
			}
...
public Action:RemoveEntity1(Handle:timer, any:entid)
{
