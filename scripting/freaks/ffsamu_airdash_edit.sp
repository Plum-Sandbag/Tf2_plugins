...
#define SAMU_STRING "ff2_airdash_HUD1"
...
new g_bDashUSEPitch[MAXPLAYERARRAY];
new Float:DashVelocity[MAXPLAYERARRAY][3];
//new g_bDashDurationInTicks[MAXPLAYERARRAY];
new g_bDashInvicibility[MAXPLAYERARRAY];
new Float:g_bDashInvicibilityDuration[MAXPLAYERARRAY];
		...
		if (IsValidClient(client))
		{
			...
			if (boss>=0)
			{
				if (FF2_HasAbility(boss, this_plugin_name, SAMU_STRING))
				{
					...
					g_bDashUSEPitch[client] = FF2_GetAbilityArgument(boss, this_plugin_name, SAMU_STRING, 25, 0);
					g_bDashInvicibility[client] = FF2_GetAbilityArgument(boss, this_plugin_name, SAMU_STRING, 26, 0);
					g_bDashInvicibilityDuration[client] = FF2_GetAbilityArgumentFloat(boss, this_plugin_name, SAMU_STRING, 27, 0.0);
//					g_bDashDurationInTicks[client] = FF2_GetAbilityArgument(boss, this_plugin_name, SAMU_STRING, 26, 0);
...
//			if((g_bClientDash[client]) && !(buttons & IN_JUMP) && (g_GlideState[client] == 2) && (g_TimeSinceDash[client] > g_TimeUntilGlide[client]))
			if((g_bClientDash[client]) && (g_GlideState[client] == 2) && (g_TimeSinceDash[client] > g_TimeUntilGlide[client]))
			{
				g_GlideState[client] = 0;
				g_GlideTime[client] = 0;
			}
			...
			if((buttons & D_Key[client]) && (g_bClientDash[client]))
			{
				if(!(g_GlideState[client]) && (g_Charges[client] > 0) && (rage >= g_fRageCost[client]) && (g_bDashReady[client]) && (g_AirTime[client] >= g_MinAirTime[client]))
				{
					...
					if(g_bDashInvicibilityDuration[client] > 0 && g_bDashInvicibility[client])
					{
						SetEntProp(client, Prop_Data, "m_takedamage", 0);
						CreateTimer(g_bDashInvicibilityDuration[client], Timer_InvincibilityEnd, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
					}

//					if(g_bDashDurationInTicks[client])
					DashVelocity[client] = forwardVector;
...
//					TeleportEntity(client, NULL_VECTOR, NULL_VECTOR, newVel);
					...
					TeleportEntity(client, NULL_VECTOR, NULL_VECTOR, DashVelocity[client]);
				}
			}
		}
	}
...
public Action:Timer_InvincibilityEnd(Handle:timer, any:userid)
{
