...
public SO_PreThink(clientIdx)
{
	if (IsLivingPlayer(clientIdx) && SO_CanUse[clientIdx] && SO_ActiveThisRound)
	{
		...
		{
			new Float:baseSpeed = SO_LowSpeed[clientIdx] + ((1.0 - (float(FF2_GetBossMax(FF2_GetBossIndex(clientIdx))) / float(SO_MaxHP[clientIdx]))) * (SO_HighSpeed[clientIdx] - SO_LowSpeed[clientIdx]));
...
public MSB_Tick(clientIdx, buttons, Float:curTime)
{
	...
	if (curTime >= MSB_UpdateHUDAt[clientIdx] && (buttons & IN_SCORE) == 0)
	{
		...
		ShowSyncHudText(clientIdx, MSB_HUDReplaceHandle, MSB_HUDReplacementFormat, FF2_GetBossCharge(bossIdx, 0), FF2_GetBossMax(bossIdx), FF2_GetBossMaxHealth(bossIdx));
	}
}
