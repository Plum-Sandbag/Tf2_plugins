...
public void TF2Items_OnGiveNamedItem_Post(int client,  char[] classname, int index, int level, int quality, int entity) //for some reason TF2CustAttr_OnKeyValuesAdded wasn't working
{
	...
	if(boss == -1)//not a boss
	{
		...
		BuildPath(Path_SM, config, sizeof(config), "data/freak_fortress_2/weapons.cfg");
		if(!FileExists(config))
		{
			PrintToServer("[CA_Weapons] Could not find data/freak_fortress_2/weapons.cfg!");
			return;
		}

		KeyValues WeaponConfig = CreateKeyValues("Weapons");
		if(!FileToKeyValues(WeaponConfig, config))
		{
			PrintToServer("[CA_Weapons] data/freak_fortress_2/weapons.cfg! is improperly formatted!");
			return;
		}
		
		if(WeaponConfig == null)//this should not be scientifically possible
			return;

		int wepIdx, wepIndex, weaponIdxcount;
		char weapon[64];
//		PrintToServer("[CUSTOM client ATTRIBUTE ADAPTER] noooo");
		for(int i=1; ; i++)
		{
			WeaponConfig.Rewind();//lets begin
			FormatEx(weapon, sizeof(weapon), "weapon%i", i);
			if(!WeaponConfig.JumpToKey(weapon, false))
			{
//				PrintToServer("[CUSTOM client ATTRIBUTE ADAPTER] no.1");
				break;
			}

			static char wepIndexStr[768];
			int isOverride = KvGetNum(WeaponConfig, "mode");
			KvGetString(WeaponConfig, "classname", weapon, sizeof(weapon));
			KvGetString(WeaponConfig, "index", wepIndexStr, sizeof(wepIndexStr));
			char buffer[8];	
			IntToString(index, buffer, sizeof(buffer));

			if(isOverride)
			{
				if(!WeaponConfig.JumpToKey("Custom Attributes", false))
				{
//					PrintToServer("[CUSTOM client ATTRIBUTE ADAPTER] no.2");
					continue;
				}
				if(StrContains(wepIndexStr, "-2")!=-1 && StrContains(classname, weapon, false)!=-1 || StrContains(wepIndexStr, "-1")!=-1 && StrEqual(classname, weapon, false))
				{
					if(isOverride != 3)
					{
						TF2CustAttr_UseKeyValues(entity, WeaponConfig);
						PrintToServer("[CUSTOM client ATTRIBUTE ADAPTER] Found custom attributes for weapon index %s. Applying now.", buffer);
						return;
					}
					else
						return;
	...
	PrintToServer("[CUSTOM ATTRIBUTE ADAPTER] Found custom attributes for weapon index %s. Applying now.", buffer);
