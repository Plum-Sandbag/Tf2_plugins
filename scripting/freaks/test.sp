...
static const String:gs_booms[][] = {"touhou/skeleton_break.wav"}; 
#define SPRITE_RED		"materials/sprites/magic_nue.vmt"
#define MODEL_ROCKET		"models/touhou/arrow.mdl"
//#define PARTICLE_ROCKET		"flamethrower_rainbow"
#define SOUND_CHAINSAW	"freak_fortress_2/ashwilliams/chainsaw.mp3"

//char MODEL_ROCKET[PLATFORM_MAX_PATH];
//char PARTICLE_ROCKET[PLATFORM_MAX_PATH];
//char SPRITE_RED[96];
...
//    PrecacheModel(PARTICLE_ROCKET, true);
...
public event_round_active(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(FF2_HasAbility( 0, this_plugin_name, "special_predator" ))
    {
        new client = GetClientOfUserId(FF2_GetBossUserId(0));					// if there are bots in the game, or the player suicides, this can be invalid... But will succeed 99% of the time
    ...
    else if(FF2_HasAbility( 0, this_plugin_name, "special_skulls" ))
    {
...
//            FF2_GetAbilityArgumentString(0, this_plugin_name, "special_skulls", 6, MODEL_ROCKET, PLATFORM_MAX_PATH);
//            FF2_GetAbilityArgumentString(0, this_plugin_name, "special_skulls", 7, PARTICLE_ROCKET, PLATFORM_MAX_PATH);
//            FF2_GetAbilityArgumentString(0, this_plugin_name, "special_skulls", 8, SPRITE_RED, 96);
    ...
    else if(FF2_HasAbility( 0, this_plugin_name, "special_ash" ))
    {
        g_bossuserid = FF2_GetBossUserId(0);
        new client = GetClientOfUserId(g_bossuserid);
        if(client && IsClientInGame(client) && IsPlayerAlive(client))
        {
            gb_Ash = true;
            g_boss = client;

            gf_CannonDamage = FF2_GetAbilityArgumentFloat(0, this_plugin_name, "special_ash", 2, 150.0);		// saw damage?
...
// add glows to clients, if you want
    ...
    else if(FF2_HasAbility( 0, this_plugin_name, "special_doom" ))				// can multiboss if you want
    {
        new client = GetClientOfUserId(FF2_GetBossUserId(0));
        if(client && IsClientInGame(client) && IsPlayerAlive(client))
        {
            ...
            if(itemflags)
            {
                ...
                g_doomRandomChance = FF2_GetAbilityArgument(0, this_plugin_name, "special_doom", 9, 5);			// random chance to get a drop
            }
            ...
            if(client > 0 && client <= client && IsClientInGame(client))
            {
                if(gf_diedFireball[client] != 0.0)									// set kill icon to hadouken
                {
                    ...
                    {			
                        ...
                        SetEventString(hEvent, "weapon", "taunt_sniper");
                        SetEventInt(hEvent, "customkill", TF_CUSTOM_TAUNT_ARROW_STAB);
                        SetEventInt(hEvent, "playerpenetratecount", 0);
...
public Action:Timer_Skulls(Handle:timer, any:userid)			// Updates boss rage stuffs
{
	...
	for (new i = 0; i < 4; i++)
	{
		if (i == 0)
			...
			classname = "tf_projectile_sentryrocket";
		else if (i == 2)
			classname = "tf_projectile_energy_ball";
		else if (i == 3)
			classname = "tf_projectile_balloffire";
...
//	for(new i; i<6; i++)
//	{
//		anglevector[0]=gf_CannonSpeed*Cosine(angles[1]/180*3.1415926+i*3.1415926/3);
//		anglevector[1]=gf_CannonSpeed*Sine(angles[1]/180*3.1415926+i*3.1415926/3);
		new target = playerarray[GetRandomInt(0, playercount-1)];
...
//    }
    ...
    TE_SetupGlowSprite(turretpos, gs_RedSprite, 0.2, 0.3, 180);
    TE_SendToAll();
}
...
CreateFireballProjectile(client, target, Float:origin[3], Float:eyeangles[3], Float:anglevector[3])
{
    ...
    if(entity != -1)
    {
        ...
        NormalizeVector(anglevector, anglevector);
        ScaleVector(anglevector, gf_CannonSpeed);
//        AttachParticle(entity, PARTICLE_ROCKET, 0.0);
//        IgniteEntity(entity, 0.0);											// only stays ignited for short time
