...
new bool:PT_Turntotarget[MAX_TURRETS];
...
new Handle:g_hCTFCreateArrow;

public OnPluginStart2()
{
	new Handle:gamedata = LoadGameConfigFile("zombie_riot");
...
//	}
	...
	new	bool:Turntotarget = FF2_GetAbilityArgument(bossIdx, this_plugin_name, PT_STRING, 19, 0) == 1;
	...
	new turret = CreateEntityByName("prop_physics_multiplayer");

	SetEntProp(turret, Prop_Data, "m_takedamage", 2);
	DispatchKeyValueVector(turret, "origin", spawnPoint);
	DispatchKeyValue(turret, "model", modelName);
	DispatchKeyValue(turret, "nodamageforces", "1");
...
//	AttachParticle(turret, "teleporter_mvm_bot_persist", 0.0,true);

//	SetEntPropEnt(turret, Prop_Send, "m_hOwnerEntity", clientIdx);
	SetEntProp(turret, Prop_Data, "m_takedamage", 2);
	...
	PT_Turntotarget[newTurretIndex] = Turntotarget;
...
public FireTurretProjectiles(turretEntity, turretIdx, dirs, type, bossEntity, Float:damage, Float:speedFactor, reskinModelIdx, bool:Turntotarget)
{
	...
	for (new i = 0; i < MAX_TURRET_FIRING_POINTS; i++)
	{
		...
		else if (PT_MotionSensor[turretIdx] == MOTION_SENSOR_ARC)
		{
			...
			if (i == TURRET_FIRE_FRONT && Turntotarget)
			{
				new Float:pos[3];
				new Float:ang[3];
				GetEntPropVector(nearestValidTarget, Prop_Send, "m_vecOrigin",pos);
				MakeVectorFromPoints(pivotOrigin, pos, pos);
				GetVectorAngles(pos, ang);
				TeleportEntity(turretEntity, NULL_VECTOR, ang, NULL_VECTOR);
...
//		damage = 120 * damageFactor;
	...
	new projectileEntity = -1;

	if(type != TURRET_TYPE_ARROW)
	{
		projectileEntity = CreateEntityByName(entname);
	...
	{
		if(g_hCTFCreateArrow)
			projectileEntity = SDKCall(g_hCTFCreateArrow, vPosition, vAngles, vVelocity, 0.1, 8, bossEntity, bossEntity);
...
//		SetEntDataFloat(projectileEntity, FindSendPropInfo(classname, "m_iDeflected") + 4, damage, true);
		//the fix
		SetEntDataFloat(projectileEntity, FindSendPropInfo("CTFProjectile_Rocket", "m_iDeflected")+4, damage, true);
...
public OnGameFrame()
{
	...
	if (PT_ActiveThisRound)
	{
		...
		for (new i = 0; i < MAX_TURRETS; i++)
		{
			...
			// fire!
			FireTurretProjectiles(turret, i, PT_TurretDirs[i], PT_TurretType[i], PT_TurretOwner[i], PT_TurretDPS[i], PT_TurretSpeedFactor[i], PT_TurretProjectileSkin[i], PT_Turntotarget[i]);
		}
	}
}
