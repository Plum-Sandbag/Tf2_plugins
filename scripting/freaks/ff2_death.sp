...
float RAPTURE_BEAM_MINS[3] = {-50.0, -50.0, 0.0};
float RAPTURE_BEAM_MAXS[3] = {50.0, 50.0, 1000.0};
...
void Rage_Mine(int iClient)
{
	...
	int iSentry = FindEntityByClassname2(-1, "obj_sentrygun");
...
void Abduct(int iClient)
{
	...
	Handle TraceRay = TR_TraceRayEx(flPos, ang, MASK_SHOT, RayType_Infinite);
	...
	if(trigger != -1)
	{
		...
		DispatchKeyValueVector(trigger, "pushdir", ang);
...
float GetMapCeiling(float flPos[3])
{
	float ang[3];
	ang[0] = 90.0; ang[1] = 0.0; ang[2] = 0.0;
	Handle hTrace = TR_TraceRayEx(flPos, ang, MASK_SHOT, RayType_Infinite);
