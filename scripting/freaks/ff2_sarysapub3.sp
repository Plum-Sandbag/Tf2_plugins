 ...
 *
 * - Updated by 93SHADoW (SHADoW), as sarysa has ceased updating any of her plugins and I have taken over updating this plugin to ensure it still functions
 */
...
AMS_ExecuteSpell(clientIdx, spellIdx)
{
	if (!IsEmptyString(AMS_CastingParticle))
	{
		...
		if (IsValidEntity(particle))
			CreateTimer(1.0, KillEnt, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration > 0.0)
			CreateTimer(duration, KillEnt, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
	return particle;
}
...
stock FullyHookedDamage(victim, inflictor, attacker, Float:damage, damageType=DMG_GENERIC, weapon=-1, Float:attackPos[3] = NULL_VECTOR)
{
	...
	if (IsValidEntity(pointHurt))
	{
		...
		KillEnt(INVALID_HANDLE, EntIndexToEntRef(pointHurt));
	}
}
...
CreateRagdoll(client, Float:flSelfDestruct=0.0, bool:isIce=false)
{
	...
	if (iRag > MaxClients && IsValidEntity(iRag))
	{
		...
		if (flSelfDestruct > 0.0)
			CreateTimer(flSelfDestruct, KillEnt, EntIndexToEntRef(iRag), TIMER_FLAG_NO_MAPCHANGE);
