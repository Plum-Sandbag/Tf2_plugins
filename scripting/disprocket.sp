...
//	HookEntityOutput("env_beam", "OnTouchedByEntity", callback);
}
...
//						if(player != client && IsClientInGame(player) && IsPlayerAlive(player))
						if(player != client && IsValidClient(player))
                        {
...
//		Format(parentName, sizeof(parentName), "skull_%d", entity);

//		DispatchKeyValue(entity, "targetname", parentName);

		new startEnt[5];
		new ent[5];
		new String:startEntName0[32],String:startEntName1[32],String:startEntName2[32],String:startEntName3[32],String:startEntName4[32];
		for(new i=0;i<5;i++)
		{

			startEnt[i] = CreateEntityByName("prop_dynamic");
			...
			switch(i)
			{
				case 0:
				{
					Format(startEntName0, sizeof(startEntName0), "start_%i", startEnt[i]);
					DispatchKeyValue(startEnt[i], "targetname", startEntName0);
				}
				case 1:
				{
					Format(startEntName1, sizeof(startEntName1), "start_%i", startEnt[i]);
					DispatchKeyValue(startEnt[i], "targetname", startEntName1);
				}
				case 2:
				{
					Format(startEntName2, sizeof(startEntName2), "start_%i", startEnt[i]);
					DispatchKeyValue(startEnt[i], "targetname", startEntName2);
				}
				case 3:
				{
					Format(startEntName3, sizeof(startEntName3), "start_%i", startEnt[i]);
					DispatchKeyValue(startEnt[i], "targetname", startEntName3);
				}
				case 4:
				{
					Format(startEntName4, sizeof(startEntName4), "start_%i", startEnt[i]);
					DispatchKeyValue(startEnt[i], "targetname", startEntName4);
				}
			}
//			DispatchKeyValue(startEnt[i], "parentname", parentName);
			DispatchKeyValue(startEnt[i], "model", MDL_ROCKET);
			DispatchKeyValue(startEnt[i], "solid", "0");
			DispatchKeyValue(startEnt[i], "rendermode", "10");
	
			DispatchSpawn(startEnt[i]);
			
			decl Float:pos0[3],Float:pos1[3];
			DotCaculate(80.0,vBuffer,pos0);
			Rotatef(pos0,origin,vBuffer,144.0*i,pos1);
			TeleportEntity(startEnt[i], pos1, vAngles, NULL_VECTOR);
//			SetVariantString(parentName);
//			AcceptEntityInput(startEnt[i], "SetParent");
			SetParentEx(entity, startEnt[i]);
		}
		for(new n=0;n<5;n++)
		{
			ent[n] = CreateEntityByName("env_beam");
			if(ent[n] != -1)
			{
				DispatchKeyValue(ent[n], "damage", "10");
				DispatchKeyValue(ent[n], "framestart", "0");
				DispatchKeyValue(ent[n], "BoltWidth", "10");
				DispatchKeyValue(ent[n], "renderfx", "0");
				DispatchKeyValue(ent[n], "TouchType", "3");
				DispatchKeyValue(ent[n], "framerate", "0");
				DispatchKeyValue(ent[n], "decalname", "Bigshot");
				DispatchKeyValue(ent[n], "TextureScroll", "50");
				DispatchKeyValue(ent[n], "HDRColorScale", "1.0");
				DispatchKeyValue(ent[n], "texture", "materials/sprites/laserbeam.vmt");
				DispatchKeyValue(ent[n], "life", "1");
				decl String:beam[30];
				Format(beam, sizeof(beam), "beam_%i", ent[n]);
				DispatchKeyValue(ent[n], "targetname", beam);
				switch(n)
				{
					case 0:
					{
						DispatchKeyValue(ent[n], "LightningStart", startEntName0);
						DispatchKeyValue(ent[n], "LightningEnd", startEntName1);

					}
					case 1:
					{
						DispatchKeyValue(ent[n], "LightningStart", startEntName1);
						DispatchKeyValue(ent[n], "LightningEnd", startEntName2);

					}
					case 2:
					{
						DispatchKeyValue(ent[n], "LightningStart", startEntName2);
						DispatchKeyValue(ent[n], "LightningEnd", startEntName3);

					}
					case 3:
					{
						DispatchKeyValue(ent[n], "LightningStart", startEntName3);
						DispatchKeyValue(ent[n], "LightningEnd", startEntName4);

					}
					case 4:
					{
						DispatchKeyValue(ent[n], "LightningStart", startEntName4);
						DispatchKeyValue(ent[n], "LightningEnd", startEntName0);

					}
				}
				DispatchKeyValue(ent[n], "spawnflags", "0");
				DispatchKeyValue(ent[n], "NoiseAmplitude", "0.0");
				DispatchKeyValue(ent[n], "rendercolor", "225 255 255");
				DispatchKeyValue(ent[n], "renderamt", "255");
				SetEntPropEnt(ent[n], Prop_Data, "m_hOwnerEntity",client);
//				SetEntityModel(ent[n], "materials/sprites/laserbeam.vmt");
				DispatchKeyValue(ent[n], "OnTouchedByEntity", "beam,TurnOff,,0.01,-1"); 
				DispatchKeyValue(ent[n], "OnTouchedByEntity", "beam,TurnOn,,0.02,-1");  
				DispatchSpawn(ent[n]);
				AcceptEntityInput(ent[n], "TurnOn");

//				SetParentEx(entity, ent[n]);
			}
		}
	}
}
...
stock bool:Rotatef(const Float:InPutDot[3],const Float:origin[3],const Float:Vector[3],const Float:angleInDegree, Float:OutPutDot[3])
{
	new Float:c, Float:s, Float:angle, Float:x, Float:y, Float:z;
	angle = DegToRad(angleInDegree);
	c = Cosine(angle);
	s = Sine(angle);
	...
	y = Vector[1];
	z = Vector[2];
	OutPutDot[0] = origin[0] + (x*x*(1-c)+c) * InPutDot[0] + (x*y*(1-c)-z*s) * InPutDot[1] + (x*z*(1-c)+y*s) * InPutDot[2];
	OutPutDot[1] = origin[1] + (y*x*(1-c)+z*s) * InPutDot[0] + (y*y*(1-c)+c) * InPutDot[1] + (y*z*(1-c)-x*s) * InPutDot[2];
	OutPutDot[2] = origin[2] + (x*z*(1-c)-y*s) * InPutDot[0] + (y*z*(1-c)+x*s) * InPutDot[1] + (z*z*(1-c)+c) * InPutDot[2];
}

stock bool:DotCaculate(const Float:Radius,const Float:Vector[3], Float:OutPutDot[3])
{
	new Float:a,Float:b,Float:c;
	a = Vector[0];
	b = Vector[1];
	c = Vector[2];
//	new Float:alpha = ArcTangent(SquareRoot(a*a+b*b)/c);
//	OutPutDot[0] = InPutDot[0] - Radius * Tangent(alpha);
//	OutPutDot[1] = InPutDot[1] - b/a * Radius * Tangent(alpha);
	if(a!=0 && b!=0)
	{
		OutPutDot[2] = Radius * SquareRoot(a*a+b*b);
		OutPutDot[1] = -1.0 * b * c * Radius / SquareRoot(a*a+b*b);
		OutPutDot[0] = -1.0 * a * c * Radius / SquareRoot(a*a+b*b);
	}
	else
	{
//		OutPutDot[2] = InPutDot[2];
//		OutPutDot[1] = InPutDot[1];
//		OutPutDot[0] = InPutDot[0] + Radius;
		OutPutDot[2] = 0.0;
		OutPutDot[1] = 0.0;
		OutPutDot[0] = -Radius;
	}
