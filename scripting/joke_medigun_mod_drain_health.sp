...
public void OnPluginStart() {
	...
	Handle dtSetChargeEffect = GetDHooksDefinition(hGameConf,
			"CTFPlayerShared::SetChargeEffect()");

	if (!dtSetChargeEffect) {
		SetFailState("Failed to setup detour for CTFPlayerShared::SetChargeEffect()");
	}

	DHookEnableDetour(dtSetChargeEffect, false, OnSetChargeEffecttPre);
...
MRESReturn OnAllowedToHealTargetPre(int medigun, Handle hReturn, Handle hParams) {
	...
	if (TF2_GetClientTeam(healer) == TF2_GetClientTeam(target))
		return MRES_Ignored;
...
MRESReturn OnSetChargeEffecttPre(Address pPlayerShared, Handle hParams) {
	int client = TF2Util_GetPlayerFromSharedAddress(pPlayerShared);
	if(DHookIsNullParam(hParams, 6))
		return MRES_Ignored;
	int healer = DHookGetParam(hParams, 6);
	if(TF2_GetClientTeam(client) != TF2_GetClientTeam(healer))
		return MRES_Supercede;
	return MRES_Ignored;
}
