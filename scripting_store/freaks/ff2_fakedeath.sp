...
char FAKE_SoundSection[MAXPLAYERARRAY][256];	// arg4
float FAKE_Speed[MAXPLAYERARRAY];				// arg5
bool FAKE_BlockAttack[MAXPLAYERARRAY];			// arg6
...
{	
	for(int i =1; i<= MaxClients; i++)
	{
		...
		SDKUnhook(i, SDKHook_PreThink, FAKE_Prethink);
...
public void HookAbilities(int bossIdx, int bossClientIdx)
{
	if(bossIdx >= 0)
	{
		if(FF2_HasAbility(bossIdx, this_plugin_name, FAKE))
		{
			...
			FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, FAKE, 4, FAKE_SoundSection[bossClientIdx], 256);
			FAKE_Speed[bossClientIdx] = FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, FAKE, 5, 340.0);
			FAKE_BlockAttack[bossClientIdx] = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, FAKE, 6, 0));
		...
		if(bossIdx >= 0)
		{
			if(FF2_HasAbility(bossIdx, this_plugin_name, FAKE))
			{
				if(FF2_GetBossLives(bossIdx) == 1 && FF2_GetBossHealth(bossIdx) <= damage) 						// if boss has one live
				{
					...
					PrintToConsoleAll("[%s] Chance: %1.f, Value: %1.f", this_plugin_name, Chance * 1.0, FAKE_Chance[victim]);
...
public void FAKE_Prethink(int bossClientIdx)
{
	if(FAKE_Speed[bossClientIdx] != -1)
		SetEntPropFloat(bossClientIdx, Prop_Send, "m_flMaxspeed", FAKE_Speed[bossClientIdx]);
		
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Dazed))
		if(GetEntProp(bossClientIdx, Prop_Send, "m_iStunFlags") & TF_STUNFLAG_SLOWDOWN)
			TF2_RemoveCondition(bossClientIdx, TFCond_Dazed);
	
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Dazed))
		if(GetEntProp(bossClientIdx, Prop_Send, "m_iStunFlags") & TF_STUNFLAG_BONKSTUCK)
			TF2_RemoveCondition(bossClientIdx, TFCond_Dazed);
			
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_OnFire))
		TF2_RemoveCondition(bossClientIdx, TFCond_OnFire);
	
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Bleeding))
		TF2_RemoveCondition(bossClientIdx, TFCond_Bleeding);
	
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Jarated))
		TF2_RemoveCondition(bossClientIdx, TFCond_Jarated);
		
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Milked))
		TF2_RemoveCondition(bossClientIdx, TFCond_Milked);
		
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_Gas))
		TF2_RemoveCondition(bossClientIdx, TFCond_Gas);
		
	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_MarkedForDeath))
		TF2_RemoveCondition(bossClientIdx, TFCond_MarkedForDeath);

	if(TF2_IsPlayerInCondition(bossClientIdx, TFCond_MarkedForDeathSilent))
		TF2_RemoveCondition(bossClientIdx, TFCond_MarkedForDeathSilent);
			
	if(GetEntProp(bossClientIdx, Prop_Send, "m_bGlowEnabled"))
		SetEntProp(bossClientIdx, Prop_Send, "m_bGlowEnabled", 0);
...
{
	if(FAKE_BlockAttack[bossClientIdx])
	{
		SetEntPropFloat(bossClientIdx, Prop_Send, "m_flNextAttack", GetGameTime() + 1000000.0); //What
	}
	
	//Start Hook
	SDKHook(bossClientIdx, SDKHook_PreThink, FAKE_Prethink);
	...
	if(GetAliveTeamCount(GetClientTeam(bossClientIdx)) <= 1) 
	{
		...
		// Clear old damagelist
		for(int p = 1; p <= MaxClients;p++)
		{
			for(int i = 0; i < strlen(DamageList[p]); i++) DamageList[p][i] = 0;
		}
		...
		for(int i = 1; i<= MaxClients;i++)
		{
			if(IsValidClient(i))
			{
				...
				if(i != bossClientIdx)
					TF2_AddCondition(i, view_as<TFCond>(38), 8.0);	// give everyone crits		
			}
		}
	}
	
	//Fake detah sound
	if(FAKE_SoundSection[bossClientIdx][0] != '\0')
	{
		FF2_EmitRandomSound(bossClientIdx, FAKE_SoundSection[bossClientIdx]);
	}

	//Drop Fake Ragdoll
	DropRagdoll(bossClientIdx);
...
public Action FakeDeath_Fix(Handle timer, int bossClientIdx)
{
	if(IsValidClient(bossClientIdx))
	{
		...
		SDKUnhook(bossClientIdx, SDKHook_PreThink, FAKE_Prethink);
		...
		FF2_StartMusic(0);
		SetEntPropFloat(bossClientIdx, Prop_Send, "m_flNextAttack", GetGameTime() + 1.0);
...
stock void DropRagdoll(int bossClientIdx)
{
	//fake ragdoll
