...
#include <morecolors> //https://raw.githubusercontent.com/shanapu/MyStore/master/scripting/include/colors.inc
...
//	HookEvent("player_mvp", Event_RoundMVP);
}
...
public bool MVPSounds_Config(KeyValues &kv, int itemid)
{
	...
	if(strlen(g_sSound[g_iCount]) > 3)
	{
		PrecacheSound(g_sSound[g_iCount], true);
		AddFileToDownloadsTable(sBuffer);
	}
...
public void OnWinPanel(Event event, const char[] name, bool dontBroadcast)
{
	...
	static char mvpname[32];
	GetClientName(mvp, mvpname, 32);

	CPrintToChatAll("正在播放 {cyan}%s {default}的MVP音乐", mvpname);
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (RoundToNearest(g_fPlayerVolume[i] * 100) != 0) // Rounded because computers struggle with floats and precise values
			EmitSoundToClient(i, g_sSound[g_iEquipt[mvp]], SOUND_FROM_PLAYER, SND_SPEAKER, SNDLEVEL_NONE, _, g_fVolume[g_iEquipt[mvp]] * g_fPlayerVolume[i]);
	}
